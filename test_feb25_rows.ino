...
void writeRows(byte receivedRows) {
  if (receivedRows & 1) {
    Serial.println("Row 1 setHigh");
    rowAct1.setHigh(pixelBrightness);
  } else {
    Serial.println("Row 1 setLow");
    rowAct1.setLow();
  }

  if (receivedRows & (1 << 1)) {
    Serial.println("Row 2 setHigh");
    rowAct2.setHigh(pixelBrightness);
  } else {
    Serial.println("Row 2 setLow");
    rowAct2.setLow();
  }
...
void actuateRow(byte rowsForCurrentColumn) {
  ...
  Serial.print("Going to write rows with integer: ");
  Serial.println(rowsForCurrentColumn);
    ...
    if (!isTimerExceeded) {
      if (endByte == END_BYTE) {
        ...
        for (int i = 0; i < NUM_COLS_TEST_GRID; i++) {
          Serial.print("Going into column: ");
          Serial.println(i+1);
